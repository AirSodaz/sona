...
pub use sona_sqlite::storage_usage::{StorageUsageSnapshot, WebviewBrowsingDataClearResult};
...
pub async fn get_usage_snapshot<R: Runtime>(
    ...
    tauri::async_runtime::spawn_blocking(move || {
        sona_sqlite::storage_usage::collect_storage_usage_snapshot(&app_local_data_dir, db.as_ref())
    })
    .await
...
pub async fn clear_webview_browsing_data<R: Runtime>(
    ...
    let after_bytes =
        sona_sqlite::storage_usage::observable_webview_cache_bytes(&app_local_data_dir)?;
    Ok(sona_sqlite::storage_usage::build_webview_clear_result(
        before_bytes,
        after_bytes,
    ))
}
