import {
  LLM_TASK_CHUNK_EVENT,
  LLM_TASK_PROGRESS_EVENT,
  LLM_TASK_TEXT_EVENT,
  LLM_TRANSCRIPT_JOB_UPDATE_EVENT,
  type LlmTaskChunkPayload,
  type LlmTaskProgressPayload,
  type LlmTaskTextPayload,
  type LlmTaskType,
  type TranscriptLlmJobResult,
} from './llmTaskTypes';
...
export class LlmTaskEvents {
  ...
  listenToTranscriptLlmJobUpdates = async (
    ...
    return this.ports.listen<TranscriptLlmJobResult>(
      LLM_TRANSCRIPT_JOB_UPDATE_EVENT,
      ({ payload }) => {
        if (payload.taskId === taskId && payload.taskType === taskType) {
