...
describe('ModelService', () => {
  ...
  describe('getModelCatalogSnapshot', () => {
    ...
    it('resolves paths and rules from the latest Rust catalog snapshot', async () => {
      ...
      await expect(modelService.getModelPath('snapshot-only-model')).resolves.toBe(
        '/snapshot/models/snapshot-only-model'
      );
  ...
  describe('Qwen3 ASR metadata', () => {
    ...
    it('registers Qwen3-ASR GGUF as a llama.cpp streaming and batch model', () => {
      ...
      const streamingModelIds = PRESET_MODELS.filter((model) =>
  ...
  describe('speaker model metadata', () => {
    ...
    it('registers the expected speaker segmentation archives and speaker embedding files', () => {
      const speakerSegmentationModels = PRESET_MODELS.filter(
        (model) => model.type === 'speaker-segmentation'
      );
      const speakerEmbeddingModels = PRESET_MODELS.filter(
        (model) => model.type === 'speaker-embedding'
      );
    ...
    it('resolves archive speaker models to directories and file speaker models to filenames', async () => {
      await expect(modelService.getModelPath('sherpa-onnx-reverb-diarization-v1')).resolves.toBe(
        '/app/data/models/sherpa-onnx-reverb-diarization-v1'
      );
      await expect(
        modelService.getModelPath('3dspeaker_speech_eres2netv2_sv_zh-cn_16k-common.onnx')
      ).resolves.toBe('/app/data/models/3dspeaker_speech_eres2netv2_sv_zh-cn_16k-common.onnx');
    });

    it('registers the expected CTC alignment model', () => {
      const alignmentModels = PRESET_MODELS.filter((model) => model.type === 'alignment');
      expect(alignmentModels.map((model) => model.id)).toEqual(['sherpa-onnx-mms-300m-fa']);
      expect(alignmentModels[0]).toEqual(
        expect.objectContaining({
          id: 'sherpa-onnx-mms-300m-fa',
          name: 'MMS-300M-FA',
          type: 'alignment',
          engine: 'sherpa-onnx',
        })
    ...
    it('adds localized speaker settings labels to both locale files', () => {
      ...
      expect(zhLocale.settings).toMatchObject({
        ...
        speaker_profiles_description:
          '从本地参考音频建立已知说话人档案库，项目可以选择启用哪些档案。',
        ...
        speaker_profile_samples_hint:
          '导入一个或多个本地参考片段。它们会被规范化为 16k 单声道 WAV，并存放在应用管理的数据目录中。',
