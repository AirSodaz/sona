...
describe('recoveryService', () => {
  ...
  it('flushes pending queue snapshot writes before saving recovered items', async () => {
    ...
    expect(testContext.invokeMock).toHaveBeenNthCalledWith(2, TauriCommand.recovery.saveSnapshot, {
      items: [],
    });
  });
  ...
  it('passes recovery items to Rust when saving so Rust owns filtering and normalization', async () => {
    ...
    expect(testContext.invokeMock).toHaveBeenCalledWith(TauriCommand.recovery.saveSnapshot, {
