...
describe('historyService', () => {
  ...
  it('saveRecording sends browser audio bytes as transport and leaves item creation to Rust', async () => {
    ...
    expect(testContext.invokeMock).toHaveBeenCalledWith('history_save_recording_to_project', {
      segments: [
  ...
  it('saveImportedFile preserves the original source path and passes converted source separately', async () => {
    ...
    expect(testContext.invokeMock).toHaveBeenCalledWith('history_save_imported_file_to_project', {
  ...
  it('forwards history audio cleanup preview and apply requests', async () => {
    ...
    expect(testContext.invokeMock).toHaveBeenNthCalledWith(1, 'history_preview_audio_cleanup', {
      retentionDays: 30,
      excludeHistoryId: 'history-open',
    });
    expect(testContext.invokeMock).toHaveBeenNthCalledWith(2, 'history_cleanup_audio', {
      retentionDays: null,
