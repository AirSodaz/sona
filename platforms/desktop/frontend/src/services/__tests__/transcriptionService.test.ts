...
vi.mock('../../stores/configStore', () => ({
  ...
  useConfigStore: {
    ...
    setState: vi.fn((updater: any) => {
      const nextState = typeof updater === 'function' ? updater({ config: mocks.config }) : updater;
      if (nextState?.config) {
...
describe('TranscriptionService voice typing diagnostics', () => {
  ...
  it('rejects when streaming ASR is not configured', async () => {
    ...
    expect(mocks.invoke).not.toHaveBeenCalledWith(
      'start_external_live_transcription',
      expect.anything()
    );
  });
  ...
  it('forwards correlated batch progress and incremental transcript events', async () => {
    ...
    await service.transcribeFile('C:/audio/demo.wav', onProgress, onSegment);
