...
export class TranscriptionService {
  ...
  private async _transcribeFileInternal(
    ...
    if (onSegment) {
      unlisten = await listen<TranscriptUpdate>(buildRecognizerOutputEvent(instanceId), (event) => {
    ...
    if (onProgress) {
      unlistenProgress = await listen<unknown>(TauriEvent.app.batchProgress, (event) => {
        ...
        if (
          path !== filePath ||
          (typeof eventInstanceId === 'string' && eventInstanceId !== instanceId)
        ) {
          return;
        }
