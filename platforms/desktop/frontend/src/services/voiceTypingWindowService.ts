...
export interface VoiceTypingWindowServicePorts {
  ...
  ) => AuxWindowController<VoiceTypingOverlayPayload>;
  createWebviewWindow: (
    label: string,
...
export function createVoiceTypingWindowService(
  ports: VoiceTypingWindowServicePorts
...
export const voiceTypingWindowService = createVoiceTypingWindowService({
  createAuxWindowController: (options) =>
    new AuxWindowController<VoiceTypingOverlayPayload>(options),
