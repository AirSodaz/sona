...
export class HistoryService {
  ...
  async completeLiveRecordingDraft(
    ...
    const item = await this.ports.historyCompleteLiveDraft(historyId, segments, duration);
  ...
  async saveNativeRecordingToProject(
    ...
    if (this.ports.historySaveRecordingToProject) {
      return normalizeHistoryItem(
        await this.ports.historySaveRecordingToProject({
          segments,
          duration,
          projectId,
          nativeAudioPath: absoluteWavPath,
  ...
  async saveImportedFileToProject(
    ...
    if (this.ports.historySaveImportedFileToProject) {
      return normalizeHistoryItem(
        await this.ports.historySaveImportedFileToProject({
          id,
          sourcePath: filePath,
          segments,
          duration,
