...
export function splitByPunctuation(segments: TranscriptSegment[]): TranscriptSegment[] {
  ...
  for (const segment of intermediateSegments) {
    const segmentText =
      typeof segment.text === 'string' ? segment.text : String(segment.text || '');
    const limit = isCJK(segmentText) ? MAX_SEGMENT_LENGTH_CJK : MAX_SEGMENT_LENGTH_WESTERN;

    if (segmentText.length > limit) {
      const subSegments = splitSegmentByRegex(segment, COMMA_SPLIT_REGEX, {
        checkAbbreviations: false,
      });
...
function splitSegmentByRegex(
  ...
  const state: SplitterState = {
    ...
    currentSegmentStart:
      hasTimestamps && tokenMap && tokenMap.timestamps.length > 0
...
function handleDelimiter(
  ...
  const fallbackSegmentEnd =
    state.currentStart +
    (totalLength > 0 ? (state.currentText.length / totalLength) * totalDuration : 0);
 ...
 */
function findTimestampFromMap(
  map: TokenMap,
  effectiveIndex: number,
  hintIndex: number = 0
): { timestamp: number; index: number } | undefined {
  // Check hint first
  if (
    hintIndex < map.startIndices.length &&
    map.startIndices[hintIndex] <= effectiveIndex &&
    effectiveIndex < map.endIndices[hintIndex]
  ) {
  ...
  if (
    next < map.startIndices.length &&
    map.startIndices[next] <= effectiveIndex &&
    effectiveIndex < map.endIndices[next]
  ) {
...
function checkHintIndex(
  segments: TranscriptSegment[],
  searchTime: number,
  time: number,
  hintIndex: number
): { segment: TranscriptSegment | undefined; index: number } | null {
...
export function alignTokensToText(
  ...
  if (
    ...
    rawTokens.length !== rawTimestamps.length
  ) {
...
export function performSegmentSplit(
  ...
  if (segment.timing?.units && segment.timing.units.length > 0) {
  ...
  if (hasLegacyTimestamps && segment.tokens && segment.timestamps) {
    ...
    if (!splitTimeFound) {
      if (rightTimestamps.length > 0) {
        ...
        splitTime =
          leftTimestamps[leftTimestamps.length - 1] + leftDurations[leftDurations.length - 1];
        splitTimeFound = true;
      }
    }
  }
