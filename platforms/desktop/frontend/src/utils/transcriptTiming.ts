...
function buildSegmentLevelTiming(
...
function buildTokenLevelTiming(
  segment: TranscriptSegment,
  source: TranscriptTimingSource
  ...
  const durations =
    Array.isArray(segment.durations) && segment.durations.length === tokens.length
...
function coerceTiming(segment: TranscriptSegment): TranscriptTiming {
  ...
  if (
    timing &&
    (timing.level === 'token' || timing.level === 'segment') &&
    (timing.source === 'model' || timing.source === 'derived')
  ) {
    ...
    if (normalizedUnits.length > 0) {
      if (!doTimingUnitsMatchSegmentText(normalizedUnits, segment.text)) {
        return (
          buildTokenLevelTiming(segment, 'model') ?? buildSegmentLevelTiming(segment, 'derived')
        );
      }
...
export function shiftTranscriptSegment(
  segment: TranscriptSegment,
...
export function normalizeTranscriptUpdate(
  update: TranscriptUpdate | TranscriptSegment | unknown
): TranscriptUpdate {
