...
describe('transcriptTextUtils', () => {
  ...
  it('aligns formatted text to token timing metadata', () => {
    const result = alignTextToTimedTokens('<b>Hello</b>, <i>world</i>', [
  ...
  it('preserves arbitrary timing payloads when tokens are aligned', () => {
    const result = alignTextToTimedTokens('Edited text', [
  ...
  it('returns the full text with fallback timing when inputs cannot be aligned', () => {
    ...
    expect(result).toEqual([{ text: 'Hello', timing: { timestamp: 5 } }]);
  });
  ...
  it('preserves old format <b> and <i> tags', () => {
    expect(sanitizeTranscriptHtml('<b>Bold</b> <i>Italic</i>')).toBe('<b>Bold</b> <i>Italic</i>');
  });
  ...
  it('preserves <s> and <code> tags while stripping non-class attributes', () => {
    ...
    expect(sanitizeTranscriptHtml('<code spellcheck="false" style="color: red">x=1</code>')).toBe(
      '<code>x=1</code>'
    );
    expect(sanitizeTranscriptHtml('<code class="editor-code" spellcheck="false">x=1</code>')).toBe(
      '<code class="editor-code">x=1</code>'
    );
  });
  ...
  it('escapes unclosed unsafe tags', () => {
    expect(sanitizeTranscriptHtml('<script src="evil.js"')).toBe('&lt;script src="evil.js"');
