...
describe('detectDownloadSource', () => {
  it('classifies source hosts', () => {
    ...
    expect(detectDownloadSource('https://raw.githubusercontent.com/org/repo/main/file')).toBe(
      'github'
    );
...
describe('applyDownloadMirror', () => {
  it('rewrites only supported hosts', () => {
    ...
    expect(applyDownloadMirror(GITHUB_URL, 'ghnet')).toBe(`https://ghproxy.net/${GITHUB_URL}`);
