...
import type { AudioRecorderLogger, InputSource, MutableRefLike, RecordSessionPhase } from './types';
...
interface CreateRecordControllerArgs {
  ...
  showError: (input: { code: string; messageKey: string; cause: unknown }) => Promise<void>;
...
export function createRecordController({
  ...
  async function startRecording(): Promise<boolean> {
    ...
    try {
      ...
      const createLiveDraft = async (audioExtension: string) => {
        const draft = await persistence.createLiveRecordingDraft(
          audioExtension,
      ...
      if (inputSource === 'desktop') {
        ...
        const nativeStarted = await capture.tryStartNativeMicrophoneCapture(sessionId, {
          deviceName:
            config.microphoneId && config.microphoneId !== 'default' ? config.microphoneId : null,
        ...
        if (!nativeStarted) {
          ...
          fallbackStream = await capture.requestWebFallbackStream(
            'microphone',
            config.microphoneId
          );
        }
  ...
  async function pauseRecording(): Promise<void> {
    ...
    try {
      ...
      logger.error(
        `[useAudioRecorder] Failed to pause recording session. session=${sessionId}:`,
        error
      );
      ...
      try {
        ...
        timing.setSegmentTimeOffsetSeconds(
          timing.getNextSegmentTimeOffsetSeconds(),
          'pause_error_recovery'
        );
      } catch (resumeError) {
        logger.warn(
      ...
      if (config.muteDuringRecording && activeInputSourceRef.current === 'microphone') {
        void capture.setSystemAudioMute(
          true,
          'Failed to remute system audio after pause rollback:'
        );
      }
  ...
  async function resumeRecording(): Promise<void> {
    ...
    try {
      ...
      logger.error(
        `[useAudioRecorder] Failed to resume recording session. session=${sessionId}:`,
        error
      );
      if (session.isActiveSession(sessionId)) {
        setIsPaused(true);
