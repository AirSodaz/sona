...
export function shouldAcceptRecordSegment(
...
export function createRecordSessionController({
  ...
  function canMutateActiveRecordResources(sessionId: string): boolean {
    return (
      refs.recordSessionIdRef.current === sessionId || refs.recordSessionIdRef.current === null
    );
  }
  ...
  function openRecordSession(): string {
    ...
    logger.info(
      `[useAudioRecorder] Record session opened. session=${sessionId} input=${getInputSource()}`
    );
    return sessionId;
  }
  ...
  function getRecordSegmentDeliveryMeta(segment: TranscriptSegment): RecordSegmentDeliveryMeta {
    ...
    return {
      ...
      accepted:
        refs.recordSessionIdRef.current !== null && shouldAcceptRecordSegment(phase, segment),
    };
  }
  ...
  async function softStopRecordSessionIfActive(sessionId: string, reason: string): Promise<void> {
    ...
    try {
      ...
      logger.info(
        `[useAudioRecorder] Rolled back recognizer state. session=${sessionId} reason=${reason}`
      );
    } catch (error) {
      logger.warn(
        `[useAudioRecorder] Failed to roll back recognizer state. session=${sessionId} reason=${reason}`,
        error
      );
    }
  }
