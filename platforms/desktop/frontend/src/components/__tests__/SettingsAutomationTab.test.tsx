  ...
  useTranslation: () => ({
    t: (key: string, options?: Record<string, unknown>) => {
      if (typeof options?.defaultValue === 'string') {
        return options.defaultValue.replace(/\{\{(\w+)\}\}/g, (_match: string, variable: string) =>
...
describe('SettingsAutomationTab', () => {
  ...
  it('hides target project when save to history is disabled', async () => {
    ...
    fireEvent.change(screen.getByPlaceholderText('e.g. Weekly Meeting Inbox'), {
      target: { value: 'Export Only Watcher' },
    });
    fireEvent.change(screen.getByPlaceholderText('Choose a folder to monitor...'), {
      target: { value: 'C:\\watch\\export_only' },
    });
    ...
    fireEvent.change(screen.getByPlaceholderText('Choose export directory...'), {
