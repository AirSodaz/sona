...
describe('Dropdown', () => {
  ...
  it('renders the dropdown with placeholder or value', () => {
    const { rerender } = render(
      <Dropdown value="" onChange={mockOnChange} options={options} placeholder="Select something" />
    );
    screen.getByText('Select something');

    rerender(<Dropdown value="option1" onChange={mockOnChange} options={options} />);
    screen.getByText('Option 1');
  });

  it('opens menu when clicked', () => {
    render(<Dropdown value="" onChange={mockOnChange} options={options} />);
  ...
  it('renders grouped options with section headers', () => {
    ...
    render(<Dropdown value="auto" onChange={mockOnChange} options={groupedOptions} />);
  ...
  describe('Search and Keyboard Loops (options.length > 10)', () => {
    ...
    it('navigates keyboard focus loop correctly', async () => {
      ...
      fireEvent.keyDown(visibleOptions[visibleOptions.length - 1], {
        key: 'ArrowDown',
        bubbles: true,
      });
      expect(document.activeElement).toBe(searchInput);
