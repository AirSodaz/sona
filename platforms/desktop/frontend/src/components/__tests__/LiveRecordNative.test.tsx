...
const { mockCreateLiveRecordingDraft, mockCompleteLiveRecordingDraft } = mockLiveRecordHistory;
...
describe('LiveRecord Native Capture', () => {
        ...
        createBuffer(channels: number, length: number, sampleRate: number) {
          ...
          return new (
            AudioBuffer as unknown as {
              new (options: { length: number; sampleRate: number }): AudioBuffer;
            }
          )({ length, sampleRate });
        }
        createBufferSource() {
  ...
  it('syncs the saved history title into the editor after native recording is persisted', async () => {
    ...
    mockCompleteLiveRecordingDraft.mockImplementationOnce((historyId: string) =>
      Promise.resolve(
        createCompletedHistoryItem(historyId, 'wav', {
          id: historyId,
    ...
    act(() => {
      useTranscriptStore.setState({
        segments: [
          { id: 'native-seg', text: 'Hello native capture', start: 0, end: 1, isFinal: true },
        ],
      });
    });
  ...
  it('persists final online streaming segments delivered during stop flush', async () => {
    mockCreateLiveRecordingDraft.mockResolvedValueOnce(
      createDraftHandle('online-history-id', 'wav')
    );
  ...
  it('should stop the correct native capture after switching desktop back to microphone', async () => {
    ...
    await act(async () => {
      fireEvent.click(screen.getByText('live.source_desktop'));
    });
    ...
    await act(async () => {
      fireEvent.click(screen.getByLabelText('live.source_select'));
    });
    await act(async () => {
      fireEvent.click(screen.getByText('live.source_microphone'));
    });
