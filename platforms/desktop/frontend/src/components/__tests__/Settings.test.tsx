...
describe('Settings', () => {
  ...
  beforeEach(() => {
    ...
    vi.mocked(modelService.getModelCatalogSnapshot).mockResolvedValue(
      createModelCatalogSnapshot() as any
    );
  ...
  async function openModelsTab() {
    ...
    await screen.findByText('settings.model_selection', undefined, {
  ...
  it('prewarms the hidden general pane without exposing a dialog or checking models', async () => {
    const { container } = render(
      <Settings isOpen={false} prewarm onClose={onClose} initialTab="models" />
    );

    const prewarmRoot = container.querySelector(
  ...
  it(
    'checks installed models only after the model settings tab is opened',
    async () => {
      render(<Settings isOpen={true} onClose={onClose} />);
  ...
  it(
    'downloads a model successfully',
    async () => {
      render(<Settings isOpen={true} onClose={onClose} />);
      await openModelsTab();
  ...
  it(
    'shows merged transcription settings inside the model settings tab',
    async () => {
      render(<Settings isOpen={true} onClose={onClose} />);
      await openModelsTab();
      ...
      expect(
        Boolean(
          vadModels.compareDocumentPosition(transcriptionSettings) &
            Node.DOCUMENT_POSITION_FOLLOWING
        )
      ).toBe(true);
      expect(
        Boolean(
          transcriptionSettings.compareDocumentPosition(restoreDefaults) &
            Node.DOCUMENT_POSITION_FOLLOWING
        )
      ).toBe(true);
  ...
  it('deletes a model', async () => {
    ...
    vi.mocked(modelService.getModelCatalogSnapshot).mockResolvedValue(
      createModelCatalogSnapshot(['test-model']) as any
    );
    ...
    await waitFor(() => {
      expect(vi.mocked(modelService.getModelCatalogSnapshot).mock.calls.length).toBeGreaterThan(
        initialCalls
      );
    });
  });
  ...
  it('loads a model path when selected from dropdown', async () => {
    ...
    await act(async () => {
      useTranscriptStore.getState().setConfig({ streamingModelPath: path, batchModelPath: path });
    });
