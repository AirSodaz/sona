...
describe('LiveRecord', () => {
  ...
  it('should start recording when Start button is clicked', async () => {
    ...
    expect(mockCreateLiveRecordingDraft).toHaveBeenCalledWith(
      'wav',
      null,
      'system:mic',
      expect.any(String)
  ...
  it('keeps transcript segment timestamps monotonic across pause and resume', async () => {
    ...
    const resumedSegment = useTranscriptStore
      .getState()
      .segments.find((segment) => segment.id === 'resume-seg');
  ...
  it('excludes paused time from the saved native recording duration', async () => {
    ...
    const lastCall =
      mockCompleteLiveRecordingDraft.mock.calls[
        mockCompleteLiveRecordingDraft.mock.calls.length - 1
      ];
  ...
  it('keeps the displayed timer accumulated across pause and resume when using web audio fallback', async () => {
    mockStartNative.mockRejectedValueOnce(new Error('native unavailable'));
  ...
  it('syncs the saved history title into the editor after web recording fallback is persisted', async () => {
    ...
    mockCompleteLiveRecordingDraft.mockImplementationOnce((historyId: string) =>
      Promise.resolve(
        createCompletedHistoryItem(historyId, 'webm', {
          id: historyId,
    ...
    mockCompleteLiveRecordingDraft.mockImplementationOnce(
      () =>
        new Promise((resolve) => {
          resolveComplete = () =>
            resolve(
              createCompletedHistoryItem('web-history-id', 'webm', {
