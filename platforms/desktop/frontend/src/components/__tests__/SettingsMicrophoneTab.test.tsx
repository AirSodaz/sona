...
describe('SettingsMicrophoneTab', () => {
  ...
  it('starts microphone preview before system-audio preview after device loading completes', async () => {
    ...
    const microphoneStartIndex = mockInvoke.mock.calls.findIndex(
      ([command]) => command === 'start_microphone_capture'
    );
    const systemStartIndex = mockInvoke.mock.calls.findIndex(
      ([command]) => command === 'start_system_audio_capture'
    );
