...
import type { RecoveredQueueItem, RecoveryItemStage, RecoverySource } from '../types/recovery';
...
export function RecoveryCenterModal({
    ...
    () => [
      {
        ...
        draftCount: grouped.batch.filter((item) => item.historyId || item.segments.length > 0)
          .length,
      ...
      },
      {
        ...
        draftCount: grouped.automation.filter((item) => item.historyId || item.segments.length > 0)
          .length,
  ...
  return (
    <PanelModal
      ...
      meta={
        <>
          <span className="panel-modal-meta-label recovery-meta-label">
            {t('recovery.labels.last_recovered')}
          </span>
        ...
        return (
          <section key={source} className="panel-modal-section recovery-section">
            <div className="panel-modal-section-header recovery-section-header">
              <div className="panel-modal-section-title recovery-section-title">
                {getSourceTitle(source, t)}
              </div>
                  ...
                  <div key={item.id} className="recovery-item-row">
                    <div className="recovery-item-main">
                      ...
                      <div className="recovery-item-meta">
                        <span className="recovery-meta-pill">{getSourceTitle(item.source, t)}</span>
                        <span className="recovery-meta-pill">
                          {t('recovery.labels.stage')}: {getStageLabel(item.lastKnownStage, t)}
                        </span>
                        <span className="recovery-meta-pill">
                          {t('recovery.labels.saved_at')}:{' '}
