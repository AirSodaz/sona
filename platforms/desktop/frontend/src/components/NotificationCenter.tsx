...
import { type TaskCenterAction, useTaskLedgerActions } from '../hooks/useTaskLedgerActions';
...
function NotificationCard({
  ...
  const copy = (
    <>
      ...
      <span className="notification-center-item-copy">
        ...
        {body ? (
          <span
            className={`notification-center-item-body${bodyClassName ? ` ${bodyClassName}` : ''}`}
          >
            {body}
          </span>
        ) : null}
  ...
  return (
    <li
      className={`notification-center-item notification-center-item-tone-${tone}${itemClassName ? ` ${itemClassName}` : ''}`}
    >
      ...
      {support ? <div className="notification-center-item-support">{support}</div> : null}
      {actions ? <div className="notification-center-item-actions">{actions}</div> : null}
    </li>
  );
}
  ...
  const renderUpdateEntry = (entry: UpdateTaskEntry) => {
    ...
    const support =
      ...
      entry.status === 'downloaded' ? (
        <>
          {entry.status === 'downloading' || entry.status === 'installing'
  ...
  const renderLedgerTask = (entry: LedgerTaskEntry) => {
    ...
    const support = hasSupport ? (
      <>
        ...
        {task.errorMessage ? (
          <div className="notification-center-item-message">{task.errorMessage}</div>
        ) : null}
      </>
    ) : null;
  ...
  const renderSection = (
    ...
    return (
      <section className={`notification-center-section notification-center-section-${section}`}>
        ...
        <ul className="notification-center-list">{sectionEntries.map(renderEntry)}</ul>
      </section>
    );
  };
        ...
        >
          ...
          {!hasEntries ? (
            ...
            <>
              {renderSection(
                'needsAction',
                t('task_center.needs_action', { defaultValue: 'Needs action' }),
                groupedEntries.needsAction
              )}
              {renderSection(
                'active',
                t('task_center.active', { defaultValue: 'Active' }),
                groupedEntries.active
              )}
              {renderSection(
                'recent',
                t('task_center.recent', { defaultValue: 'Recent' }),
                groupedEntries.recent
              )}
            </>
          )}
        </div>
