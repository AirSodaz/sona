...
const RETENTION_PRESETS = [
  {
    value: 'forever',
    days: null,
    labelKey: 'settings.storage.retention_forever',
    defaultLabel: 'Keep forever',
  },
  {
    value: '0',
    days: 0,
    labelKey: 'settings.storage.retention_immediate',
    defaultLabel: 'Remove immediately',
  },
  { value: '7', days: 7, labelKey: 'settings.storage.retention_7_days', defaultLabel: '7 days' },
  {
    value: '30',
    days: 30,
    labelKey: 'settings.storage.retention_30_days',
    defaultLabel: '30 days',
  },
  {
    value: '90',
    days: 90,
    labelKey: 'settings.storage.retention_90_days',
    defaultLabel: '90 days',
  },
  {
    value: '180',
    days: 180,
    labelKey: 'settings.storage.retention_180_days',
    defaultLabel: '180 days',
  },
  {
    value: '365',
    days: 365,
    labelKey: 'settings.storage.retention_365_days',
    defaultLabel: '365 days',
  },
] as const;
...
function formatFileCount(count: number, t: ReturnType<typeof useTranslation>['t']): string {
...
function formatReportSummary(
  ...
  if (report.removedCount > 0) {
    parts.push(
      t('settings.storage.result_removed', {
  ...
  if (report.removedBytes > 0) {
    parts.push(
      t('settings.storage.result_bytes', {
  ...
  if (report.missingMarkedCount > 0) {
    parts.push(
      t('settings.storage.result_missing', {
  ...
  if (report.failedCount > 0) {
    parts.push(
      t('settings.storage.result_failed', {
  ...
  if (report.skippedActiveCount > 0) {
    parts.push(
      t('settings.storage.result_skipped_active', {
...
export function SettingsStorageTab(): React.JSX.Element {
  ...
  const handleChangeDataDir = async () => {
    try {
      ...
      const confirmed = await confirm(
        ...
        {
          title: t('settings.storage.data_dir_change_confirm_title', {
  ...
  const handleResetModelsDir = async () => {
    ...
    try {
      const confirmed = await confirm(
        ...
        {
          title: t('settings.storage.models_dir_reset_confirm_title', {
            defaultValue: 'Restore Default Models Directory?',
          }),
  ...
  const handleCleanNow = async () => {
    ...
    if (retentionDays === null) {
      setResultMessage(
        t('settings.storage.cleanup_disabled', {
    ...
    try {
      ...
      const confirmed = await confirm(
        t('settings.storage.cleanup_confirm_message', {
          defaultValue:
            'This removes managed history audio attachments only. Transcripts, summaries, and history records stay intact.',
        }),
        {
          title: t('settings.storage.cleanup_confirm_title', {
            defaultValue: 'Clean history audio?',
          }),
  ...
  const handleClearWebviewData = async () => {
    const confirmed = await confirm(
      t('settings.storage.webview_clear_confirm_message', {
        defaultValue:
          'This clears WebView cache and browsing data through the system WebView API. Lightweight UI state stored by the WebView may also reset.',
      }),
      {
        title: t('settings.storage.webview_clear_confirm_title', {
          defaultValue: 'Clear WebView browsing data?',
        }),
        details: t('settings.storage.webview_clear_confirm_details', {
          defaultValue:
            'Audio, database files, models, and app configuration are not deleted. Some space may only be released after restarting Sona.',
        }),
  ...
  const manualCleanupDisabled =
    isCleaning || historyAudioRetentionDays === null || historyAudioRetentionDays === undefined;
  const webviewClearDisabled =
    isClearingWebview || !usageSnapshot?.categories.webviewCache.clearSupported;
    ...
    <SettingsTabContainer id="settings-panel-storage" ariaLabelledby="settings-tab-storage">
      <SettingsPageHeader
        ...
        description={t('settings.storage.description', {
          defaultValue:
            'Review local data usage and clean managed storage without deleting transcript text.',
        })}
      />
      ...
      <SettingsSection
        ...
        description={t('settings.storage.locations_description', {
          defaultValue:
            'Manage the storage directories for application data and AI speech recognition models.',
        })}
        icon={<Folder size={20} />}
      >
        <div className="settings-storage-locations-grid">
          <SettingsLocationCard
            ...
            hint={t('settings.storage.data_dir_hint', {
              defaultValue:
                'Stores SQLite databases, history recordings, speaker voice profiles, and recovery snapshots. Changing this requires restarting Sona.',
            })}
          ...
          <SettingsLocationCard
            ...
            hint={t('settings.storage.models_dir_hint', {
              defaultValue:
                'Stores offline ASR speech recognition, VAD, and punctuation models. Newly downloaded models will be saved here.',
            })}
      ...
      <SettingsSection
        ...
        description={t('settings.storage.usage_description', {
          defaultValue:
            'Storage is grouped by local audio, SQLite database files, models, temporary files, WebView cache, and other app data.',
        })}
        ...
        <div className="settings-storage-usage-panel">
          ...
          {usageError && (
            <div className="settings-storage-usage-error" role="alert">
              <strong>
                {t('settings.storage.usage_error_title', {
                  defaultValue: 'Storage usage unavailable',
                })}
              </strong>
              ...
              <div className="settings-storage-usage-grid">
                <UsageMetric
                  ...
                  detail={t('settings.storage.audio_detail', {
                    ...
                    history: formatBytes(
                      usageSnapshot.categories.audio.historyAudioBytes,
                      i18n.language
                    ),
                    samples: formatBytes(
                      usageSnapshot.categories.audio.speakerSampleBytes,
                      i18n.language
                    ),
                  })}
                />
                ...
                <UsageMetric
                  icon={<Globe2 size={18} />}
                  title={t('settings.storage.category_webview_cache', {
                    defaultValue: 'WebView Cache',
                  })}
                  value={formatOptionalBytes(
                    usageSnapshot.categories.webviewCache.bytes,
                    t,
                    i18n.language
                  )}
                  detail={
                    usageSnapshot.categories.webviewCache.path
                      ? t('settings.storage.webview_observed', {
                          defaultValue: 'Observable cache directory',
                        })
                      : t('settings.storage.webview_unobserved', {
                          defaultValue: 'Size unavailable on this platform',
                        })
                  }
                />
                <UsageMetric
              ...
              <div className="settings-storage-detail-grid">
                <div className="settings-storage-detail-block">
                  <div className="settings-storage-detail-heading">
                    {t('settings.storage.database_breakdown_title', {
                      defaultValue: 'SQLite breakdown',
                    })}
                  </div>
                  <StorageDetailRow
                  ...
                  />
                  <StorageDetailRow
                    label={t('settings.storage.sqlite_index_bytes', {
                      defaultValue: 'SQLite indexes',
                    })}
                    value={sqlite ? formatBytes(sqlite.indexBytes, i18n.language) : '0 B'}
                  />
                  <StorageDetailRow
                    label={t('settings.storage.sqlite_free_page_bytes', {
                      defaultValue: 'Free pages',
                    })}
                    value={sqlite ? formatBytes(sqlite.freePageBytes, i18n.language) : '0 B'}
                  />
                  <StorageDetailRow
                    label={t('settings.storage.sqlite_index_entries', {
                      defaultValue: 'Indexed structures',
                    })}
                  ...
                  />
                  <StorageDetailRow
                    label={t('settings.storage.sqlite_main_files', {
                      defaultValue: 'Main DB + WAL/SHM',
                    })}
                    value={
                      sqlite
                        ? formatBytes(
                            sqlite.mainDbBytes + sqlite.mainWalBytes + sqlite.mainShmBytes,
                            i18n.language
                          )
                        : '0 B'
                    }
                  />
                  <StorageDetailRow
                    label={t('settings.storage.sqlite_analytics_files', {
                      defaultValue: 'Analytics DB + WAL/SHM',
                    })}
                    value={
                      sqlite
                        ? formatBytes(
                            sqlite.analyticsDbBytes +
                              sqlite.analyticsWalBytes +
                              sqlite.analyticsShmBytes,
                            i18n.language
                          )
                        : '0 B'
                    }
                ...
                <div className="settings-storage-detail-block">
                  <div className="settings-storage-detail-heading">
                    {t('settings.storage.webview_title', {
                      defaultValue: 'WebView cache and browsing data',
                    })}
                  </div>
                  <p className="settings-storage-detail-copy">
                    {t('settings.storage.webview_description', {
                      defaultValue:
                        'Clears WebView cache and browsing data through Tauri. Some space may only be released after restarting Sona.',
                    })}
                  </p>
                  <button
                    type="button"
      ...
      <SettingsSection
        ...
        description={t('settings.storage.history_audio_description', {
          defaultValue:
            'Cleanup affects only managed audio files. Saved transcripts, summaries, and history rows remain available.',
        })}
        icon={<HardDrive size={20} />}
      >
        <SettingsItem
          ...
          hint={t('settings.storage.retention_hint', {
            defaultValue:
              'Automatic cleanup runs after startup and then at most once per day while Sona stays open.',
          })}
        >
          <div className="settings-storage-retention-control">
            <Dropdown
              id="settings-history-audio-retention"
              aria-label={t('settings.storage.retention_label', {
        ...
        <SettingsItem
          ...
          hint={
            manualCleanupDisabled && !isCleaning
              ...
              : t('settings.storage.manual_cleanup_hint', {
                  defaultValue:
                    'Preview eligible audio first, then confirm before anything is removed.',
                })
          }
        >
          <button
