import { FileAudio, SlidersHorizontal, Volume2 } from 'lucide-react';
...
import { getPathStatuses, getRuntimeEnvironmentStatus } from '../../services/tauri/app';
import {
...
export function SettingsMicrophoneTab({
  ...
  const { startVisualizer: startMicWaveAnimation, stopVisualizer: stopMicWaveAnimation } =
    useAudioVisualizer({
      canvasRef,
  ...
  const { startVisualizer: startSystemWaveAnimation, stopVisualizer: stopSystemWaveAnimation } =
    useAudioVisualizer({
  ...
  const stopPreviewCapture = useCallback(
    async (kind: PreviewCaptureKind, instanceId: 'test_mic' | 'test_system') => {
      const path =
        kind === 'microphone'
  ...
  const startMicrophonePreview = useCallback(
    async (deviceId: string, isCurrentRequest: () => boolean) => {
      if (!isCurrentRequest()) {
        return;
      }
  ...
  const startSystemPreview = useCallback(
    async (deviceId: string, isCurrentRequest: () => boolean) => {
      if (!isCurrentRequest()) {
        return;
      }
  ...
  useEffect(() => {
    ...
    const isCurrentSystemRequest = () =>
      isMounted && systemRequestId === systemPreviewRequestIdRef.current;
