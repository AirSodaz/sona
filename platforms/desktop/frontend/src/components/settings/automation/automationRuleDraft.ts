...
export function normalizeAutomationRuleDraft(draft: AutomationRuleDraft): AutomationRuleDraft {
  const normalizedMode = normalizeExportMode(
    draft.stageConfig.autoTranslate,
    draft.exportConfig.mode
  );
...
export function createRuleDraft(
  projectId = 'inbox',
  kind: NonNullable<AutomationRule['kind']> = 'file'
): AutomationRuleDraft {
...
export function createDraftFromRule(rule: AutomationRule): AutomationRuleDraft {
  return normalizeAutomationRuleDraft({
    ...
    profileSource:
      rule.profileSource ?? ((rule.kind ?? 'file') === 'file' ? 'explicit' : 'tag_match'),
