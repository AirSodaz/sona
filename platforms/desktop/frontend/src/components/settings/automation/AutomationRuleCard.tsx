import { ArrowRight, ChevronDown, ChevronRight, FolderSync, Sparkles, Tags } from 'lucide-react';
...
export function AutomationRuleCard({
  ...
  const resultChipTone =
    resultLabel === t('automation.last_result_success', { defaultValue: 'Success' })
      ? 'success'
  ...
  const isFileRule =
    typeLabel === t('automation.file_rule', { defaultValue: 'File' }) ||
    watchDirectory !== undefined;
  const isWatching =
    enabled &&
    (statusLabel === t('automation.status_watching', { defaultValue: 'Watching' }) ||
      statusLabel === 'Watching');
  const isError =
    (failureCount || 0) > 0 ||
    statusLabel === 'Error' ||
    statusLabel === t('automation.status_error', { defaultValue: 'Error' });
        ...
        >
          ...
          <div className="automation-card-flow-body">
            <span className="automation-flow-node-box node-source" title={watchDirectory}>
              ...
              <span>
                {watchDirectory
                  ? watchDirectory.split(/[/\\]/).pop() || watchDirectory
                  : t('automation.none', { defaultValue: 'None' })}
              </span>
            </span>

            <span className="automation-flow-arrow-icon">
              <ArrowRight size={13} />
            </span>
          ...
          <div className="automation-card-meta-row">
            {watchDirectory !== undefined && (
              <div className="settings-item-hint">
                {t('automation.watch_directory', { defaultValue: 'Watch Directory' })}:{' '}
                {watchDirectory || t('automation.none', { defaultValue: 'None' })}
              </div>
            )}
            {outputDirectory !== undefined && (
              <div className="settings-item-hint">
                {t('automation.output_directory', { defaultValue: 'Output Directory' })}:{' '}
                {outputDirectory || t('automation.none', { defaultValue: 'None' })}
              </div>
            )}
            {resultMessage && <div className="settings-item-hint">{resultMessage}</div>}
            {blockedHint && (
              <div
        ...
        <div className="automation-card-controls">
          ...
          {onScanNow && (
            <button
              ...
              onClick={onScanNow}
              title={t('automation.scan_now', { defaultValue: 'Scan Now' })}
            >
              <PlayIcon />
          ...
          {onRetryFailed && (
            <button
              ...
              title={t('automation.retry_failed', { defaultValue: 'Retry Failed' })}
            >
              <PauseIcon />
          ...
          {onApplyExisting && (
            <button
              ...
              title={t('automation.apply_existing', { defaultValue: 'Apply to existing' })}
            >
              <PlayIcon />
