import { Activity, FolderSync, Sparkles } from 'lucide-react';
...
import type { AutomationRule, AutomationRuntimeStatus } from '../../types/automation';
import {
...
export function SettingsAutomationTab(): React.JSX.Element {
  ...
  const getRuntimeBlockedReasonLabel = (reason: string | undefined) => {
    switch (reason) {
      case 'already_processed':
        return t('automation.blocked_reason_already_processed', {
          defaultValue: 'already processed',
        });
      case 'already_pending':
        return t('automation.blocked_reason_already_pending', { defaultValue: 'already queued' });
      case 'recovery_blocked':
        return t('automation.blocked_reason_recovery_blocked', {
          defaultValue: 'blocked by recovery',
        });
      case 'project_missing':
        return t('automation.blocked_reason_project_missing', {
          defaultValue: 'target project is missing',
        });
      case 'retry_source_missing':
        return t('automation.blocked_reason_retry_source_missing', {
          defaultValue: 'retry source is unavailable',
        });
      default:
        return null;
    }
  ...
  const createEditor = (key: string, draft: AutomationRuleDraft) => (
    <AutomationRuleEditor
      draft={draft}
      onBrowseDirectory={(field) => {
        void handleBrowseDirectory(key, field);
      }}
  ...
  const activeWatchingCount = rules.filter(
    (r) => r.enabled && (r.kind ?? 'file') === 'file'
  ).length;
    ...
    <SettingsTabContainer id="settings-panel-automation" ariaLabelledby="settings-tab-automation">
      <SettingsPageHeader
        ...
        description={t('automation.description', {
          defaultValue:
            'Watch local folders for new audio files, automatically ingest them into target projects, and run their deterministic pipelines.',
        })}
      />
      ...
      <div className="automation-stats-banner">
        <div className="automation-stat-item">
          ...
          <div className="automation-stat-copy">
            ...
            <span className="automation-stat-label">
              {t('automation.stat_watchers', { defaultValue: 'Active Folder Watchers' })}
            </span>
          </div>
        </div>
        ...
        <div className="automation-stat-item">
          ...
          <div className="automation-stat-copy">
            <span className="automation-stat-value">
              {totalActiveRulesCount} / {rules.length}
            </span>
            <span className="automation-stat-label">
              {t('automation.stat_rules', { defaultValue: 'Enabled Rules' })}
            </span>
          </div>
        </div>
        ...
        <div className="automation-stat-item">
          ...
          <div className="automation-stat-copy">
            ...
            <span className="automation-stat-label">
              {t('automation.stat_processed', { defaultValue: 'Processed Files' })}
            </span>
          </div>
        </div>
      </div>
      ...
      >
        <div className="settings-item-container layout-horizontal">
          <div className="settings-item-info">
            <div className="settings-item-title">
              {t('automation.rule_count', {
                defaultValue: '{{count}} rules configured.',
            ...
            <div className="settings-item-hint">
              {t('automation.file_list_hint', {
                defaultValue:
                  'Each watched folder forwards new audio files directly to its target project pipeline.',
              })}
            </div>
          </div>
        ...
        {newRuleDraft && (
          <AutomationRuleCard
            title={
              newRuleDraft.name.trim() ||
              t('automation.create_rule', { defaultValue: 'Create Rule' })
            }
            typeLabel={t('automation.file_rule', { defaultValue: 'Folder' })}
            projectLabel={
              projectOptions.find((o) => o.value === (newRuleDraft.projectId || 'inbox'))?.label ||
        ...
        {visibleRules.length === 0 && !newRuleDraft ? (
            ...
            <div className="settings-item-info">
              ...
              <div className="settings-item-hint">
                {t('automation.empty_hint', {
                  defaultValue:
                    'Set up a watched directory to automatically transcribe and process files.',
                })}
              </div>
            </div>
          </div>
          ...
          visibleRules.map((rule) => {
            ...
            const targetProject = projects.find(
              (p) => p.id === (displayRule.projectId || displayRule.tagIds?.[0])
            );
            ...
            return (
              <AutomationRuleCard
                ...
                outputDirectory={
                  displayRule.saveHistory ? undefined : displayRule.exportConfig?.directory
                }
                watchDirectory={displayRule.watchDirectory}
                enabled={displayRule.enabled}
                canToggle={true}
                onToggleEnabled={(value) => {
                  void handleToggleEnabled(rule.id, value);
                }}
                ...
                onScanNow={() => {
                  void handleScanNow(rule.id);
                }}
                onRetryFailed={() => {
                  void handleRetryFailed(rule.id);
                }}
                onDelete={() => {
