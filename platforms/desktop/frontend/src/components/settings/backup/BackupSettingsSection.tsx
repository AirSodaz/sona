...
export function BackupSettingsSection({
    ...
    if (current && isPresetShrink(current, preset)) {
      confirmShrink = await confirm(
        t('settings.sync.shrink_confirm', {
          defaultValue:
            'Shrinking the preset publishes tombstones for data that leaves sync. Continue?',
        }),
        {
  ...
  const handleChangeMasterPassword = (
    ...
    runAction('change_password', async () => {
      await changeSyncMasterPassword({ currentMasterPassword, nextMasterPassword });
      await alert(
        t('settings.sync.password_changed', { defaultValue: 'Master password changed.' }),
        { variant: 'success' }
      );
    });
  ...
  const sectionDescription =
    status.state === 'disabled'
      ? t('settings.sync.description_disabled', {
          defaultValue: 'Encrypted multi-device sync is off for this data directory.',
        })
      : t('settings.sync.description_enabled', {
          defaultValue:
            'End-to-end encrypted incremental sync. Audio and device-local settings stay local.',
        });
