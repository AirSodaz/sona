...
export function useBackupSettingsController(): UseBackupSettingsControllerResult {
  ...
  const backupBlockerHint =
    blocker === 'recording'
      ...
      : blocker === 'batch'
        ...
        : t('settings.backup.advanced_hint', {
            defaultValue: 'Export or replace local data using a complete recovery archive.',
          });
  ...
  const confirmImport = React.useCallback(
    async (prepared: PreparedBackupImport) =>
      confirm(
    ...
    async () =>
      runAction('import', async () => {
        await runPreparedBackupImportFlow({
          ...
          onError: (error) =>
            reportError('backup.import_failed', 'errors.backup.import_failed', error),
        });
      }),
