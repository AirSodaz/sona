...
import { STORE_KEY_BACKUP_WEBDAV, settingsStore } from '../../../services/storageService';
import {
...
export function LegacyRemoteBackupPanel({
  ...
  React.useEffect(() => {
    let cancelled = false;
    void settingsStore.get<BackupWebDavConfig | null>(STORE_KEY_BACKUP_WEBDAV).then((legacy) => {
      if (!cancelled && legacy) {
        setConfig({
  ...
  const restore = async (entry: LegacyRemoteBackupEntry) => {
    ...
    try {
      await runPreparedBackupImportFlow({
        ...
        confirm: (prepared: PreparedBackupImport) =>
          confirm(
  ...
  return (
    <div className="sync-legacy-panel">
      <div className="sync-form-heading">
        ...
        <span>{t('settings.sync.legacy_title', { defaultValue: 'Legacy WebDAV archives' })}</span>
      </div>
      <div className="settings-accordion-copy-hint">
        {t('settings.sync.legacy_hint', {
          defaultValue:
            'Import an existing remote archive once. New full-archive uploads are no longer created.',
        })}
      </div>
      ...
      <button
        ...
        disabled={disabled || busy !== null || !isReady(config)}
      >
      ...
      {loaded ? (
        entries.length === 0 ? (
          <div className="sync-empty-state">
            {t('settings.sync.legacy_empty', { defaultValue: 'No legacy backup archives found' })}
          </div>
        ) : (
                ...
                <span>
                  ...
                  <small>
                    {entry.modifiedAt
                      ...
                      : t('settings.sync.unknown_time', { defaultValue: 'Unknown time' })}{' '}
