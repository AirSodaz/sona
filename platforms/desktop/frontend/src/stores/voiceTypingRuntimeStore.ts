...
function withTimestamp<T extends Partial<VoiceTypingRuntimeStatus>>(
  patch: T
): T & {
  updatedAt: number;
} {
...
export const useVoiceTypingRuntimeStore = create<VoiceTypingRuntimeStore>((set) => ({
    ...
    set((state) => {
      ...
      return withTimestamp({
        ...
        ...(status === 'idle' && state.lastErrorSource === 'shortcut_registration'
          ...
          : {}),
        ...(status === 'ready'
          ? maybeClearRecoveredFailure({ ...state, shortcutRegistration: status })
          : {}),
      });
    }),
  ...
  reportRuntimeError: (source, errorMessage) =>
    set(() =>
      withTimestamp({
        ...(source === 'shortcut_registration' ? { shortcutRegistration: 'error' as const } : {}),
        ...(source === 'warmup' || source === 'microphone' ? { warmup: 'error' as const } : {}),
  ...
  clearRuntimeFailure: (options) =>
    set(() =>
      withTimestamp({
        lastErrorSource: null,
        lastErrorMessage: null,
        ...(options?.resetShortcutRegistration ? { shortcutRegistration: 'idle' as const } : {}),
