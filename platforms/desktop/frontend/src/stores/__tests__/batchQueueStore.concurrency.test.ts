...
describe('batchQueueStore Concurrency', () => {
  ...
  it('should process multiple items up to maxConcurrent', async () => {
    ...
    const task1 =
      Promise.withResolvers<
        Array<{ id: string; start: number; end: number; text: string; isFinal: boolean }>
      >();
    const task2 =
      Promise.withResolvers<
        Array<{ id: string; start: number; end: number; text: string; isFinal: boolean }>
    ...
    const tasks: Record<
      string,
      Promise<Array<{ id: string; start: number; end: number; text: string; isFinal: boolean }>>
    > = {
    ...
    expect(
      useBatchQueueStore.getState().queueItems.filter((i) => i.status === 'pending').length
    ).toBe(1);
  ...
  it('should respect maxConcurrent config change', async () => {
    ...
    vi.mocked(transcriptionService.transcribeFile).mockImplementation(
      () => deferred.promise as unknown as never
    );
