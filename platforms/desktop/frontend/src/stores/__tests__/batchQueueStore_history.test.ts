...
describe('batchQueueStore History Integration', () => {
  ...
  it('saves Volcengine batch imports from the original file path without a temp WAV copy', async () => {
    ...
    expect(useBatchQueueStore.getState().queueItems[0]).toEqual(
  ...
  it('keeps the active editor title aligned with the saved history title for batch imports', async () => {
    ...
    const mockSegments = [{ id: 'seg1', start: 0, end: 1, text: 'Hello', isFinal: true }];
    ...
    vi.mocked(transcriptionService.transcribeFile).mockResolvedValue(mockSegments);
    vi.mocked(historyService.saveImportedFile).mockImplementation(
      () => savePromise as unknown as never
    );
    useBatchQueueStore.getState().addFiles([file]);
  ...
  it('syncs the active editor audio URL from the saved history item after batch import completes', async () => {
    ...
    vi.mocked(historyService.getAudioUrl).mockResolvedValueOnce(
      'asset:///history/history-player.wav'
    );
    ...
    expect(historyService.getAudioUrl).toHaveBeenCalledWith('history-player');
    expect(useTranscriptStore.getState()).toEqual(
      expect.objectContaining({
    ...
    useBatchQueueStore.getState().enqueueRecoveredItems([
      {
        ...
        resolution: 'pending',
        progress: 45,
        segments: [{ id: 'draft-1', start: 0, end: 1, text: 'Draft text', isFinal: true }],
        projectId: null,
        historyId: 'history-1',
  ...
  it('passes the latest known stage through settled automation success payloads', async () => {
    ...
    expect(emitAutomationTaskSettled).toHaveBeenCalledWith(
      expect.objectContaining({
        ruleId: 'rule-1',
        sourceFingerprint: 'fp-success',
        status: 'complete',
  ...
  it('passes the latest known stage through settled automation failure payloads', async () => {
    ...
    vi.mocked(transcriptionService.transcribeFile).mockRejectedValue(
