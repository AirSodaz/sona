...
vi.mock('../../services/recoveryService', () => ({
  ...
  markAutomationRecoveryItemsResumed: (...args: unknown[]) =>
    testContext.markAutomationRecoveryItemsResumedMock(...args),
  persistQueueRecoverySnapshot: (...args: unknown[]) =>
    testContext.persistQueueRecoverySnapshotMock(...args),
  flushRecoverySnapshotWrites: (...args: unknown[]) =>
    testContext.flushRecoverySnapshotWritesMock(...args),
...
vi.mock('../automationStore', () => ({
  useAutomationStore: {
    getState: () => ({
      markRecoveryItemDiscarded: (...args: unknown[]) =>
        testContext.markRecoveryItemDiscardedMock(...args),
    }),
  },
}));
...
vi.mock('../../services/taskLedgerBuilders', () => ({
  ...
  buildRecoveryTaskLedgerRecord: (...args: unknown[]) =>
    Reflect.apply(testContext.buildRecoveryTaskLedgerRecordMock, undefined, args),
  upsertTaskLedgerRecord: (...args: unknown[]) =>
    Reflect.apply(testContext.upsertTaskLedgerRecordMock, undefined, args),
  patchTaskLedgerRecord: (...args: unknown[]) =>
    Reflect.apply(testContext.patchTaskLedgerRecordMock, undefined, args),
  removeTaskLedgerRecord: (...args: unknown[]) =>
    Reflect.apply(testContext.removeTaskLedgerRecordMock, undefined, args),
...
describe('recoveryStore', () => {
  ...
  it('loads an empty snapshot without showing recovery items', async () => {
    ...
    expect(useRecoveryStore.getState()).toEqual(
  ...
  it('mirrors pending recovery items into the task ledger', async () => {
    ...
    expect(testContext.upsertTaskLedgerRecordMock).toHaveBeenCalledWith(
      expect.objectContaining({
  ...
  it('resumes one recovery item from the task center', async () => {
    ...
    expect(testContext.patchTaskLedgerRecordMock).not.toHaveBeenCalledWith(
      'recovery-recovery-single',
      expect.objectContaining({
        status: 'succeeded',
      })
    );
  ...
  it('keeps a recovery ledger item recoverable when resume fails', async () => {
    ...
    await expect(useRecoveryStore.getState().resumeItem('recovery-failed-resume')).rejects.toThrow(
      'Queue unavailable.'
    );

    expect(testContext.saveRecoveredItemsMock).not.toHaveBeenCalled();
    expect(testContext.removeTaskLedgerRecordMock).not.toHaveBeenCalledWith(
      'recovery-recovery-failed-resume'
    );
    expect(testContext.patchTaskLedgerRecordMock).toHaveBeenCalledWith(
      'recovery-recovery-failed-resume',
      expect.objectContaining({
        status: 'recoverable',
        retryable: true,
  ...
  it('discards automation recovery items and persists the remaining list', async () => {
    ...
    expect(testContext.removeTaskLedgerRecordMock).toHaveBeenCalledWith(
      'batch-recovery-automation-1'
    );
    expect(testContext.removeTaskLedgerRecordMock).toHaveBeenCalledWith(
      'recovery-recovery-automation-1'
    );
  ...
  it('keeps a recovery ledger item recoverable when discard fails', async () => {
    ...
    await expect(
      useRecoveryStore.getState().discardItem('recovery-failed-discard')
    ).rejects.toThrow('Disk full.');

    expect(testContext.removeTaskLedgerRecordMock).not.toHaveBeenCalledWith(
      'recovery-recovery-failed-discard'
    );
    expect(testContext.patchTaskLedgerRecordMock).toHaveBeenCalledWith(
      'recovery-recovery-failed-discard',
      expect.objectContaining({
