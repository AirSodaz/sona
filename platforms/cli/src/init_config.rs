...
fn generate_config_content() -> String {
    generate_config_content_inner(crate::desktop_paths::default_models_dir())
}
...
mod tests {
    ...
    fn template_contains_transcribe_and_serve_keys() {
        let content = generate_config_content_inner(None);
        for key in [
            "model_id",
            "vad_model_id",
    ...
    fn generated_config_uses_forward_slashes_for_model_path() {
        let content = generate_config_content_inner(Some(PathBuf::from("C:\\Users\\test\\models")));
