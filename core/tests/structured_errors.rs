...
use sona_core::ports::path::{PathKind, PathPortError};
...
fn path_and_clock_errors_keep_machine_readable_context() {
    let path_error = PathPortError::new(PathKind::AppLocalData, "scope denied");
...
fn domain_errors_distinguish_repository_serialization_and_clock_failures() {
    ...
    let recovery = RecoveryError::Path(PathPortError::new(PathKind::AppLocalData, "recovery path"));
