...
test('desktop filesystem adapters live in platform rather than repositories', () => {
  ...
  assert.match(cliCargo, /^sona-export\s*=/mu);
  ...
  assert.match(exportAdapter, /impl TranscriptExportRepository for FsTranscriptExportRepository/u);
  assert.match(exportCommand, /ExportService::new\(FsTranscriptExportRepository\)/u);
  assert.match(exportCommand, /tauri::async_runtime::spawn_blocking/u);
  assert.doesNotMatch(exportCommand, /adapter_export_transcript_file/u);
