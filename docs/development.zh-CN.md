...
- Rust stable 工具链
- 用于构建 llama.cpp 本地 ASR 适配器的 CMake 与 C/C++ 编译器（推荐 Clang 工具链，如 Linux 下 `clang`/`clang++`、Windows 下 `clang-cl` + Ninja）。桌面端与 CLI 构建会动态链接生成的 ggml 与 llama 共享库，发布安装包会将这些库暂存到宿主运行时目录。
