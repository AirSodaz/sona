...
use super::types::{AsrMode, AsrTranscriptionRequest, BatchTranscriptionRequest, TranscriptSegment};
...
impl AsrProviderAdapter for LocalSherpaAdapter {
    ...
    async fn create_streaming_session(
        ...
        } = request.engine_config.clone()
        {
            ...
            Err(SherpaError::Generic("Expected LocalSherpa engine config".to_string()))
        }
    }
}
    ...
    async fn process_file(
        ...
        let config = match request.engine_config.clone() {
            crate::asr::types::AsrEngineConfig::LocalSherpa {
                ...
                postprocessor: TranscriptPostprocessor::compile(request.postprocess_options.clone())?,
                gpu_acceleration,
            },
            _ => return Err(SherpaError::Generic("Expected LocalSherpa engine config".to_string())),
        };
