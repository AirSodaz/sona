...
fn lowercase_with_original_boundaries(text: &str) -> (String, Vec<LowercaseBoundary>) {
    ...
    for (original_start, ch) in text.char_indices() {
        ...
        for lower_ch in ch.to_lowercase() {
            ...
            boundaries.push(LowercaseBoundary {
                lower_start,
                lower_end: lowered.len(),
                original_start,
                original_end,
            });
        }
...
fn original_start_for_lower_byte(
    boundaries: &[LowercaseBoundary],
    lower_byte: usize,
    original_len: usize,
) -> usize {
    for boundary in boundaries {
        if lower_byte <= boundary.lower_start || lower_byte < boundary.lower_end {
            return boundary.original_start;
        }
    }
    original_len
}

fn original_end_for_lower_byte(
    boundaries: &[LowercaseBoundary],
    lower_byte: usize,
    original_len: usize,
) -> usize {
    for boundary in boundaries {
        if lower_byte <= boundary.lower_start {
            return boundary.original_start;
        }
        if lower_byte <= boundary.lower_end {
            return boundary.original_end;
        }
    }
    original_len
}

fn replace_literal_ignore_case(text: &str, from: &str, to: &str) -> String {
    let (lowered_text, boundaries) = lowercase_with_original_boundaries(text);
    let lowered_from = from.to_lowercase();
    if lowered_from.is_empty() {
        return text.to_string();
    }

    let mut result = String::new();
    let mut lower_search_start = 0usize;
    let mut original_tail_start = 0usize;
    let mut replaced_any = false;

    while let Some(relative_start) = lowered_text[lower_search_start..].find(&lowered_from) {
        let lower_match_start = lower_search_start + relative_start;
        let lower_match_end = lower_match_start + lowered_from.len();
        let original_match_start =
            original_start_for_lower_byte(&boundaries, lower_match_start, text.len());
        let original_match_end =
            original_end_for_lower_byte(&boundaries, lower_match_end, text.len());

        if original_match_start >= original_tail_start && original_match_end >= original_match_start
        {
            result.push_str(&text[original_tail_start..original_match_start]);
            result.push_str(to);
            original_tail_start = original_match_end;
            replaced_any = true;
        }

        lower_search_start = lower_match_end;
    }
    ...
    result.push_str(&text[original_tail_start..]);
    result
}

fn apply_transcript_text_replacements(
    text: &str,
    sets: &[TranscriptTextReplacementRuleSet],
) -> String {
    if text.is_empty() || sets.is_empty() {
        return text.to_string();
    }

    let mut result = text.to_string();
    for rule in active_text_replacement_rules(sets) {
        if rule.ignore_case {
            result = replace_literal_ignore_case(&result, &rule.from, &rule.to);
        } else {
...
pub(crate) fn postprocess_transcript_segments(
    ...
    segments
        ...
        .filter_map(|mut segment| {
            if options.drop_final_dot_segments && segment.is_final && segment.text == "." {
                return None;
            }
            segment.text =
                apply_transcript_text_replacements(&segment.text, &options.text_replacement_sets);
            Some(segment)
        })
        .collect()
}

pub(crate) fn postprocess_transcript_update(
...
#[cfg(test)]
mod tests {
    use super::super::types::TranscriptTextReplacementRule;
    use super::*;
    ...
    fn postprocess_segments_replaces_enabled_rules_like_frontend() {
        let options = TranscriptPostprocessOptions {
        ...
        let processed = postprocess_transcript_segments(
            vec![sample_segment(
    ...
    fn postprocess_segments_drops_only_exact_final_dot_segments() {
        ...
        let processed =
            postprocess_transcript_segments(vec![partial_dot, final_dot, spaced_dot], &options);
