...
fn default_vad_model_id() -> String { "silero-vad".to_string() }

#[derive(Serialize)]
#[serde(tag = "type", rename_all = "lowercase")]
pub enum ServerMessage {
    Started { session_id: String },
    Segment { segment: crate::sherpa::TranscriptSegment },
    Stopped,
...
pub async fn handle_streaming(
    ...
    let permit = state.streaming_semaphore.clone().try_acquire_owned().map_err(|_| StatusCode::SERVICE_UNAVAILABLE)?;
...
async fn handle_streaming_socket(mut socket: WebSocket, state: ServerState, _permit: tokio::sync::OwnedSemaphorePermit) {
    let session_id = uuid::Uuid::new_v4().to_string();

    // Phase 1: Wait for Start message
    let start_msg = match tokio::time::timeout(std::time::Duration::from_secs(10), socket.recv()).await {
        Ok(Some(Ok(Message::Text(text)))) => {
            match serde_json::from_str::<ClientMessage>(&text) {
                Ok(ClientMessage::Start { model_id, language, hotwords, vad_model_id }) => {
                    (model_id, language, hotwords, vad_model_id)
                }
                _ => {
                    let _ = socket.send(Message::Text(serde_json::to_string(&ServerMessage::Error { message: "Expected start message".to_string() }).unwrap().into())).await;
                    return;
                }
            }
        }
    ...
    let recognizer = match load_recognizer(&state, &model_id, &language, hotwords).await {
        Ok(r) => r,
        Err(e) => {
            let _ = socket.send(Message::Text(serde_json::to_string(&ServerMessage::Error { message: e }).unwrap().into())).await;
            return;
        }
    };
    ...
    let vad = match crate::sherpa::load_vad(Some(vad_model_id)) {
        Some(v) => v,
        None => {
            let _ = socket.send(Message::Text(serde_json::to_string(&ServerMessage::Error { message: "VAD model required but not found".to_string() }).unwrap().into())).await;
            return;
        }
    };

    let _ = socket.send(Message::Text(serde_json::to_string(&ServerMessage::Started { session_id: session_id.clone() }).unwrap().into())).await;
...
async fn load_recognizer(state: &ServerState, model_id: &str, language: &str, hotwords: Option<String>) -> Result<Arc<crate::sherpa::Recognizer>, String> {
...
fn run_offline_inference_standalone(
    ...
    if speech_buffer.is_empty() { return None; }
    let crate::sherpa::RecognizerInner::Offline(r) = &recognizer.inner else { return None; };
    ...
    if let Some(result) = stream.get_result() {
        ...
        if cleaned_text.is_empty() { return None; }
        ...
        let timestamps_abs: Option<Vec<f32>> = result.timestamps.as_ref()
            .map(|ts| ts.iter().map(|t| *t + global_start as f32).collect());
        let durations = timestamps_abs.as_ref()
