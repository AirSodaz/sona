...
describe('voiceTypingWindowService', () => {
    ...
    it('prepares a hidden overlay window ahead of time with the standard size', async () => {
        ...
        expect(mocks.createdWindows[0].setSize).not.toHaveBeenCalled();
        expect(mocks.createdWindows[0].setPosition).not.toHaveBeenCalled();
        expect(mocks.createdWindows[0].hide).not.toHaveBeenCalled();
    });
