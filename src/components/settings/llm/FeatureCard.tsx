...
export const FeatureCard = React.memo(function FeatureCard({
  ...
  if (!featureEnabled) {
    statusBadge = <span className="status-badge off"><X size={12}/> {t('settings.llm.status_off')}</span>;
  } else if (isComplete) {
    statusBadge = <span className="status-badge ready"><Check size={12}/> {t('settings.llm.status_ready')}</span>;
  } else {
    let missingMessage = t('settings.llm.status_missing_model');
    if (selectedProvider && localModelName) {
      missingMessage = t('settings.llm.status_missing_api_key');
    }
    statusBadge = (
      <span className="status-badge missing">
        <X size={12}/> {missingMessage}
      </span>
    );
  }
