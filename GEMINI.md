...
- **API Service:** The Sona API service supports local sherpa-onnx models and Cloud ASR models (Volcengine Doubao, Groq Whisper, Mistral Voxtral). In API requests, `model_id` accepts cloud provider IDs, and supports both batch and streaming modes (where supported by the provider).
...
### Development
- `pnpm run tauri dev` â€?Start the full desktop application in development mode.
- `pnpm run dev` â€?Run the Vite development server only (port 1420).

### Testing
- `pnpm test` â€?Run the Vitest unit/component test suite.
- `pnpm exec vitest src/path/to/file.test.tsx` â€?Run a specific Vitest file.
- `pnpm exec playwright test` â€?Run Playwright end-to-end tests.
- `cargo test --manifest-path src-tauri/Cargo.toml` â€?Run Rust backend tests.
...
- `src/` â€?React frontend source code.
  - `components/` â€?UI components. Key component: `TranscriptWorkbench.tsx` (the core editor view).
  - `hooks/` â€?React hooks for logic and lifecycle.
  - `services/` â€?Orchestration for transcription, LLM, history, and models.
  - `stores/` â€?Zustand store definitions (`transcriptSessionStore`, `transcriptPlaybackStore`, `transcriptRuntimeStore`, `transcriptSidecarStore`, `effectiveConfigStore`, `historyStore`, `dialogStore`, etc.).
  - `locales/` â€?i18n translation files (EN/ZH).
- `src-tauri/` â€?Rust backend and Tauri configuration.
- `scripts/` â€?Build and setup helper scripts.
