...
- `src/` ?React frontend source code.
  ...
  - `stores/` ?Zustand store definitions (`transcriptSessionStore`, `transcriptPlaybackStore`, `effectiveConfigStore`, `historyStore`, `dialogStore`, `automationStore`, `batchQueueStore`, `onboardingStore`, etc.).
  - `locales/` ?i18n translation files (EN/ZH).
  - `types/` ?Shared TypeScript definitions.
  - `utils/` ?Shared utility functions.
- `src-tauri/` ?Rust backend and Tauri configuration.
  - `src/app/` ?System-level app management (tray, window, settings).
  - `src/cli/` ?CLI subcommand implementations.
  - `src/commands/` ?Tauri command handlers.
  - `src/core/` ?Business logic (pipeline, automation, diagnostics, preset models).
  - `src/integrations/` ?External adapters (ASR, LLM, WebDAV, Audio).
