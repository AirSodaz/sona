...
mod tests {
    ...
    #[test]
    fn a_handle_and_the_free_functions_observe_the_same_state() {
        let directory = tempfile::tempdir().unwrap();
        let app_data_dir = directory.path().to_string_lossy().into_owned();
        let context = SonaContext::open(app_data_dir.clone()).unwrap();

        // Written through the handle...
        context
            .create_tag_json(r#"{"name":"through-handle"}"#.to_string())
            .unwrap();

        // ...and read back through the legacy free function.
        let via_free_function = crate::load_tag_repository_state_json(app_data_dir).unwrap();

        assert!(
            via_free_function.contains("through-handle"),
            "both surfaces must reach one context: {via_free_function}"
        );
    }

    #[test]
    fn a_handle_reports_the_normalized_directory_not_the_string_it_was_given() {
        let directory = tempfile::tempdir().unwrap();
        let nested = directory.path().join("child");
        std::fs::create_dir_all(&nested).unwrap();
        let alias = nested.join("..");

        let via_alias = SonaContext::open(alias.to_string_lossy().into_owned()).unwrap();
        let direct = SonaContext::open(directory.path().to_string_lossy().into_owned()).unwrap();

        // The registry canonicalizes, so a path alias resolves to one directory
        // — compared handle to handle rather than against a hardcoded form,
        // because the canonical shape is platform specific.
        assert_eq!(via_alias.app_data_dir(), direct.app_data_dir());
        assert!(Arc::ptr_eq(&via_alias.inner, &direct.inner));
        assert_ne!(via_alias.app_data_dir(), alias.to_string_lossy());
    }

    #[test]
    fn two_handles_for_one_directory_share_a_context() {
        let directory = tempfile::tempdir().unwrap();
        let app_data_dir = directory.path().to_string_lossy().into_owned();

        let first = SonaContext::open(app_data_dir.clone()).unwrap();
        let second = SonaContext::open(app_data_dir).unwrap();
