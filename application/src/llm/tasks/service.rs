...
pub enum LlmTaskResult {
    Polish(Vec<sona_core::llm::tasks::PolishedSegment>),
    Translate(Vec<sona_core::llm::tasks::TranslatedSegment>),
    Summary(sona_core::llm::tasks::TranscriptSummaryResult),
}

#[derive(Clone, Debug, PartialEq, Eq, Serialize)]
#[serde(tag = "type", content = "payload", rename_all = "snake_case")]
pub enum LlmTaskEvent {
    Progress(sona_core::llm::tasks::LlmTaskProgressPayload),
    PolishChunk(sona_core::llm::tasks::LlmTaskChunkPayload<sona_core::llm::tasks::PolishedSegment>),
    TranslateChunk(
        sona_core::llm::tasks::LlmTaskChunkPayload<sona_core::llm::tasks::TranslatedSegment>,
    ),
    SummaryChunk(LlmTaskSummaryChunkPayload),
    Text(sona_core::llm::tasks::LlmTaskTextPayload),
    ...
    Runtime: LlmTaskRuntimePort,
{
    ...
    pub async fn polish(
        ...
        let budget = sona_core::llm::tasks::resolve_task_budget(
            model.as_ref(),
            sona_core::llm::tasks::DEFAULT_SEGMENT_PROMPT_CHAR_BUDGET,
            sona_core::llm::tasks::estimate_structured_output_tokens(&request.segments),
        );
        let chunks = sona_core::llm::tasks::plan_segment_chunks_with_budget(
            &request.segments,
            request.chunk_size,
            budget,
            |segments| {
        ...
        while let Some(result) = pending.next().await {
            ...
            emit(
                observer,
                LlmTaskEvent::PolishChunk(sona_core::llm::tasks::LlmTaskChunkPayload {
                    task_id: request.task_id.clone(),
                    task_type: sona_core::llm::tasks::LlmTaskType::Polish,
            ...
            emit_progress(
                observer,
                &request.task_id,
                sona_core::llm::tasks::LlmTaskType::Polish,
                completed,
                total_chunks,
            )?;
    ...
    pub async fn translate(
        ...
        let free_translation = request.config.strategy
        ...
        while let Some(result) = pending.next().await {
            ...
            emit(
                observer,
                LlmTaskEvent::TranslateChunk(sona_core::llm::tasks::LlmTaskChunkPayload {
                    task_id: request.task_id.clone(),
                    task_type: sona_core::llm::tasks::LlmTaskType::Translate,
                    chunk_index: portable_chunk_counter("chunk index", chunk_index + 1)?,
                    total_chunks: portable_chunk_counter("total chunks", total_chunks)?,
                    items: items.clone(),
                }),
            ...
            emit_progress(
                observer,
                &request.task_id,
                sona_core::llm::tasks::LlmTaskType::Translate,
                completed,
                total_chunks,
            )?;
            ordered[chunk_index] = Some(items);
    ...
    pub async fn summarize(
        ...
        observer: &dyn LlmTaskObserver,
    ) -> Result<sona_core::llm::tasks::TranscriptSummaryResult, LlmTaskError> {
        ...
        let explicit_budget = request
            ...
            .unwrap_or(sona_core::llm::tasks::DEFAULT_SUMMARY_CHUNK_CHAR_BUDGET);
        let map_budget = sona_core::llm::tasks::resolve_task_budget(
            model.as_ref(),
            explicit_budget,
            DEFAULT_SUMMARY_MAP_OUTPUT_TOKENS,
        );
        let final_budget = sona_core::llm::tasks::resolve_task_budget(
            model.as_ref(),
            sona_core::llm::tasks::DEFAULT_SEGMENT_CONTEXT_CHAR_BUDGET,
            DEFAULT_SUMMARY_FINAL_OUTPUT_TOKENS,
        );
        let chunk_char_budget = sona_core::llm::tasks::resolve_summary_chunk_char_budget(
            &request.template,
            explicit_budget,
            final_budget,
        ...
        let chunks =
            sona_core::llm::tasks::split_summary_segments(&request.segments, chunk_char_budget);

        if chunks.len() == 1 {
            let prompt =
                sona_core::llm::tasks::build_summary_direct_prompt(&request.template, &chunks[0]);
            if !sona_core::llm::tasks::summary_prompt_fits_budget(
                sona_core::llm::tasks::SUMMARY_FINAL_SYSTEM_PROMPT,
                &prompt,
                final_budget,
            ) {
                return Err(LlmTaskError::InvalidRequest {
            ...
            emit_progress(
                observer,
                &request.task_id,
                sona_core::llm::tasks::LlmTaskType::Summary,
                1,
                1,
            )?;
        ...
        let mut pending = stream::iter(chunks.into_iter().enumerate())
            .map(|(chunk_index, chunk)| {
                let config = config.clone();
                let prompt = sona_core::llm::tasks::build_summary_chunk_prompt(
                    template,
                    &chunk,
                    chunk_index + 1,
                    map_count,
                ...
                async move {
                    self.complete_text(
                        config,
                        sona_core::llm::tasks::SUMMARY_MAP_SYSTEM_PROMPT,
                        prompt,
        ...
        let response = self
            .stream_summary(
                &request,
                sona_core::llm::tasks::build_summary_finalize_prompt(&request.template, &partials),
                LlmPromptCachePolicy::Automatic,
                final_budget.max_output_tokens,
                observer,
            )
    ...
    async fn translate_direct_chunk(
        ...
        if translations.len() != expected.len() {
            return Err(LlmTaskError::InvalidResponse {
                reason: sona_core::llm::tasks::chunk_error(
                    sona_core::llm::tasks::LlmTaskType::Translate,
                    chunk_number,
                    format!(
        ...
        Ok(expected
            ...
            .zip(translations)
            .map(
                |(segment, translation)| sona_core::llm::tasks::TranslatedSegment {
                    id: segment.id,
                    translation,
                },
            )
    ...
    async fn reduce_summary_partials(
        ...
        budget: sona_core::llm::tasks::LlmTaskBudget,
        ...
        for _ in 0..MAX_SUMMARY_REDUCE_ROUNDS {
            ...
            let previous_chars = partials
                ...
                .sum::<usize>();
            let groups = sona_core::llm::tasks::split_summary_partials_for_reduce(
                &request.template,
                &partials,
                budget,
            );
                ...
                observer
                    .on_event(LlmTaskEvent::Text(
                        sona_core::llm::tasks::LlmTaskTextPayload {
                            ...
                            task_type: sona_core::llm::tasks::LlmTaskType::Summary,
                            text: delta.text,
                            delta: delta.delta,
                            reset: false,
                        },
...
fn validate_segment_inputs(
    segments: &[sona_core::llm::tasks::LlmSegmentInput],
) -> Result<(), LlmTaskError> {
...
fn summary_reduce_output_tokens(budget: sona_core::llm::tasks::LlmTaskBudget) -> Option<u64> {
    let target = budget
        .prompt_token_budget
...
fn emit_progress(
    observer: &dyn LlmTaskObserver,
    task_id: &str,
    task_type: sona_core::llm::tasks::LlmTaskType,
    completed_chunks: usize,
    total_chunks: usize,
) -> Result<(), LlmTaskError> {
    emit(
        observer,
        LlmTaskEvent::Progress(sona_core::llm::tasks::LlmTaskProgressPayload {
            task_id: task_id.to_string(),
            task_type,
...
#[cfg(test)]
mod tests {
    use super::portable_chunk_counter;
    use sona_core::llm::tasks::LlmTaskError;
